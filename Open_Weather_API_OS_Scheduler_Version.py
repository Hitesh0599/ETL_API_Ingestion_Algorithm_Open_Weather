...
# ## 2.0 Defining Paths and API Key
...
api_key = "05a8a82be991a50e86a83c9eb6d64e32"
...
def fn_errors(response):
    error_dict = {
        ...
        "404" : "City name or Lat/Long are incorrect",
    ...
    return(error_dict.get(code, f"Error Code: {code}"))
...
def find_existing_weather_record(data, Output_df, updation_row):
    ...
    country = fn_complex_dictionary_parsing_function(data, "['sys']['country']")
    city = fn_complex_dictionary_parsing_function(data, "name")
    ...
    if len(Output_df.loc[mask,]) > 0:
        updation_row = Output_df.loc[mask,].index
...
def fn_saving_final_output(data, Unit_Dict, unit_type, Output_folder):
    ...
    output_column_list = ["Country", "City", "Latitude", "Longitude", "Weather", "Description", "Temperature", "Temp Range", "Humidity", "Wind Speed", "Unit Type", "Weather Timestamp"]
    if has_output == 0:
    ...
    updation_row = find_existing_weather_record(data, Output_df, updation_row)
...
if os.path.exists(os.path.join(Input_folder, "City Names.xlsx")):
    ...
    for city_name in city_names_list:
        ...
        if isinstance(data, dict):
            fn_saving_final_output(data, Unit_Dict, unit_type, Output_folder)
...
if os.path.exists(os.path.join(Input_folder, "Receiver Info.xlsx")):
    app_password = "ewue chly uyxo kais"
